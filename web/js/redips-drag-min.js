...
Jan 25, 2011.
*/
"use strict";var REDIPS=REDIPS||{};REDIPS.drag=(function(){var init,init_tables,enable_drag,img_onmousemove,handler_onmousedown,table_top,handler_onmouseup,handler_onmousemove,handler_onresize,set_trc,set_color,box_offset,calculate_cells,getScrollPosition,autoscrollX,autoscrollY,clone_object,clone_limit,elementControl,trash_delete,get_style,save_content,move_object,obj_margin=null,window_width=0,window_height=0,scroll_width=null,scroll_height=null,edge={page:{x:0,y:0},div:{x:0,y:0},flag:{x:0,y:0}},scroll_object,bgcolor_old=null,scrollable_container=[],tables=[],moved_flag=0,cloned_flag=0,cloned_id=[],currentCell=[],div_drag=null,div_box=null,top_position=0,table=null,table_old=null,table_source=null,row=null,row_old=null,row_source=null,cell=null,cell_old=null,cell_source=null,obj=false,obj_old=false,hover_color='#E7AB83',bound=25,speed=20,only={div:[],cname:'only',other:'deny'},mark={action:'deny',cname:'mark',exception:[]},border='solid',border_disabled='dotted',trash='trash',trash_ask=true,drop_option='multiple',delete_cloned=true,source_cell=null,current_cell=null,previous_cell=null,target_cell=null,clone_ctrlKey=false;init=function(dd){var self=this,i,imgs,evt,obj_new_div;if(dd===undefined){dd='drag';}
...
init_tables();handler_onresize();REDIPS.event.add(window,'resize',handler_onresize);enable_drag('init');imgs=div_drag.getElementsByTagName('img');for(i=0;i<imgs.length;i++){REDIPS.event.add(imgs[i],'mousemove',img_onmousemove);}
...
return-1;};}};init_tables=function(){var i,element,nested,tables_nodeList;tables_nodeList=div_drag.getElementsByTagName('table');for(i=0;i<tables_nodeList.length;i++){element=tables_nodeList[i].parentNode;do{if(element===div_drag){nested=0;break;}
if(element.nodeName==='TD'){nested=1;element.table=true;top_position++;break;}
element=element.parentNode;}while(element);tables[i]=tables_nodeList[i];tables[i].nested=nested;tables[i].idx=i;}
tables.sort(function(a,b){return b.nested-a.nested;});};img_onmousemove=function(){return false;};handler_onmousedown=function(e){var evt=e||window.event,offset,mouseButton,position;if(elementControl(evt)){return true;}
...
if(mouseButton===1){moved_flag=0;cloned_flag=0;REDIPS.event.add(document,'mousemove',handler_onmousemove);REDIPS.event.add(document,'mouseup',handler_onmouseup);REDIPS.drag.myhandler_clicked();if(obj.setCapture){obj.setCapture();}}
...
idx=tables.indexOf(e);if(e.nested===1&&idx!==0){tmp=tables[0];tables[0]=tables[idx];tables[idx]=tmp;}
else if(e.nested===0&&idx!==top_position){tmp=tables[top_position];tables[top_position]=tables[idx];tables[idx]=tmp;}};handler_onmouseup=function(e){var evt=e||window.event,target_table,i,target_elements,target_elements_length;if(obj.releaseCapture){obj.releaseCapture();}
REDIPS.event.remove(document,'mousemove',handler_onmousemove);REDIPS.event.remove(document,'mouseup',handler_onmouseup);div_drag.onselectstart=null;obj.style.left=0;obj.style.top=0;obj.style.zIndex=10;obj.style.position='static';scroll_width=document.documentElement.scrollWidth;scroll_height=document.documentElement.scrollHeight;edge.flag.x=edge.flag.y=0;if(cloned_flag===1&&(table===null||row===null||cell===null)){obj.parentNode.removeChild(obj);cloned_id[obj_old.id]-=1;REDIPS.drag.myhandler_notcloned();}
...
scrollPosition=getScrollPosition()[0];edge.page.x*=evt.clientX<window_width/2?-1:1;if(!((edge.page.x<0&&scrollPosition<=0)||(edge.page.x>0&&scrollPosition>=(scroll_width-window_width)))){if(edge.flag.x++===0){REDIPS.event.remove(window,'scroll',calculate_cells);autoscrollX(window);}}}
else{edge.page.x=0;}
...
scrollPosition=getScrollPosition()[1];edge.page.y*=evt.clientY<window_height/2?-1:1;if(!((edge.page.y<0&&scrollPosition<=0)||(edge.page.y>0&&scrollPosition>=(scroll_height-window_height)))){if(edge.flag.y++===0){REDIPS.event.remove(window,'scroll',calculate_cells);autoscrollY(window);}}}
else{edge.page.y=0;}
...
edge.div.x*=evt.clientX<sca.midstX?-1:1;if(edge.flag.x++===0){REDIPS.event.remove(sca.div,'scroll',calculate_cells);autoscrollX(sca.div);}}
else{edge.div.x=0;}
...
edge.div.y*=evt.clientY<sca.midstY?-1:1;if(edge.flag.y++===0){REDIPS.event.remove(sca.div,'scroll',calculate_cells);autoscrollY(sca.div);}}
else{edge.div.y=0;}
break;}
...
else{calculate_cells();REDIPS.event.add(scroll_object,'scroll',calculate_cells);edge.flag.x=0;currentCell=[0,0,0,0];}};autoscrollY=function(so){var pos,old,scrollPosition,maxsp,edgeCrossed;if(typeof(so)==='object'){scroll_object=so;}
...
else{calculate_cells();REDIPS.event.add(scroll_object,'scroll',calculate_cells);edge.flag.y=0;currentCell=[0,0,0,0];}};clone_object=function(){var obj_new=obj.cloneNode(true),offset,offset_dragged;document.getElementById('obj_new').appendChild(obj_new);if(obj_new.setCapture){obj_new.setCapture();}
...
REDIPS.drag.myhandler_undeleted();}};enable_drag=function(enable_flag,div_id){var i,j,divs=[],borderStyle,cursor,overflow,enabled,cb,handler,position,regex_drag=/\bdrag\b/i;if(enable_flag===true||enable_flag==='init'){handler=handler_onmousedown;borderStyle=REDIPS.drag.border;cursor='move';enabled=true;}
...
for(i=0,j=0;i<divs.length;i=i+1){if(regex_drag.test(divs[i].className)){divs[i].onmousedown=handler;divs[i].style.borderStyle=borderStyle;divs[i].style.cursor=cursor;divs[i].redips_enabled=enabled;divs[i].redips_container=div_drag;}
else if(enable_flag==='init'){overflow=get_style(divs[i],'overflow');if(overflow!=='visible'){REDIPS.event.add(divs[i],'scroll',calculate_cells);position=get_style(divs[i],'position');cb=box_offset(divs[i],position,false);scrollable_container[j]={div:divs[i],offset:cb,midstX:(cb[1]+cb[3])/2,midstY:(cb[0]+cb[2])/2};j++;}}}};get_style=function(el,style_name){var val;if(el.currentStyle){val=el.currentStyle[style_name];}
